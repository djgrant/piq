...
| --- | --- |
| `pok docs dev` | Dev server on http://localhost:3002 |
| `pok docs build` | Production build into `docs/dist` |
| `pok docs deploy` | Build, then deploy the `piq-docs` Cloudflare Worker |

## Layout

| Path | Purpose |
| --- | --- |
| `manual/`, `packages/` | Markdown, one directory per docs category |
...
cd - && pnpm --filter @piqit/docs-site update @notation/docs
```

Because that path does not exist on CI, the workflows install with
`--filter '!@piqit/docs-site'`. Once the framework is published, swap the `file:`
